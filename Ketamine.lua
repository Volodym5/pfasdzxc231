...
return function(shared, page)
    ...
    local function appendLog() -- never call that function inside the hooked function, because it will lack capabilities and cause an error
        ...
        if event:IsA("BindableFunction") then
            ...
            if got and got[1] then
                ...
                if log:FindFirstChild("_LogText") then log._LogText.Value = sArgs end
            end
        end
    end
    ...
    addButton("Copy all logs", function()
        local parts = {}
        for _, log in logs do
            local sv = log:FindFirstChild("_LogText")
            if sv then
                parts[#parts + 1] = sv.Value
            end
        end
        if #parts > 0 then
            clipboard(table.concat(parts, "\n\n-- ============ --\n\n"))
        end
    end)
...
return function(shared, page)
    ...
    local function addLog(realUrl, method, func, headers, body, result)
        ...
        log.LayoutOrder = order
        local _hsv = Instance.new("StringValue")
        _hsv.Name = "_LogURL"
        _hsv.Value = url  -- full URL
        _hsv.Parent = log
    ...
    addButton("Copy all logs", function()
        local parts = {}
        for _, log in logs do
            local sv = log:FindFirstChild("_LogURL")
            if sv then
                parts[#parts + 1] = sv.Value
            end
        end
...
return function(shared, page)
    ...
    local function appendLog() -- never call that function inside the hooked function, because it will lack capabilities and cause an error
        ...
        log.Visible = true
        log.LayoutOrder = order
        local _rsv = Instance.new("StringValue")
        _rsv.Name = "_LogText"
        _rsv.Value = sArgs
        _rsv.Parent = log
    ...
    addButton("Copy all logs (Outgoing)", function()
        local parts = {}
        for _, log in logs.To do
            local sv = log:FindFirstChild("_LogText")
            if sv then parts[#parts + 1] = sv.Value end
        end
        if #parts > 0 then
            clipboard(table.concat(parts, "\n\n-- ============ --\n\n"))
        end
    ...
    addButton("Copy all logs (Incoming)", function()
        local parts = {}
        for _, log in logs.From do
            local sv = log:FindFirstChild("_LogText")
            if sv then parts[#parts + 1] = sv.Value end
    ...
    addButton("Copy all logs (All)", function()
        local parts = {}
        for _, log in logs.To do
            local sv = log:FindFirstChild("_LogText")
            if sv then parts[#parts + 1] = "-- [OUTGOING]\n" .. sv.Value end
        end
        for _, log in logs.From do
            local sv = log:FindFirstChild("_LogText")
            if sv then parts[#parts + 1] = "-- [INCOMING]\n" .. sv.Value end
        end
        if #parts > 0 then
            clipboard(table.concat(parts, "\n\n-- ============ --\n\n"))
        end
