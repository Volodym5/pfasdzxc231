...
local Library = {
    ...
    NotifyTweenInfo = TweenInfo.new(0.22, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
    ...
    InputThrottleDuration = 1 / 90,

    IsLightTheme = false,
    Scheme = {
        BackgroundColor = Color3.fromRGB(18, 18, 23),
        MainColor = Color3.fromRGB(30, 30, 39),
        AccentColor = Color3.fromRGB(125, 85, 255),
        OutlineColor = Color3.fromRGB(60, 60, 70),
        FontColor = Color3.fromRGB(225, 225, 235),
        Font = Font.fromEnum(Enum.Font.Code),

        RedColor = Color3.fromRGB(255, 102, 102),
...
local function PlayTween(PreviousTween: TweenBase?, Instance: Instance, TweenInfoObject: TweenInfo, Goals: { [string]: any })
    if PreviousTween then
        StopTween(PreviousTween)
    end

    local NewTween = TweenService:Create(Instance, TweenInfoObject, Goals)
    NewTween:Play()
    return NewTween
end

local function ThrottleInput(LastInputTime: number)
    local CurrentTime = tick()
    if CurrentTime - LastInputTime < Library.InputThrottleDuration then
        return false, LastInputTime
    end
...
function Library:MakeDraggable(UI: GuiObject, DragFrame: GuiObject, IgnoreToggled: boolean?, IsMainWindow: boolean?)
    ...
    Library:GiveSignal(UserInputService.InputChanged:Connect(function(Input: InputObject)
        ...
        if Dragging and IsHoverInput(Input) then
            local CanProceed
            CanProceed, LastDragInput = ThrottleInput(LastDragInput)
...
function Library:MakeResizable(UI: GuiObject, DragFrame: GuiObject, Callback: () -> ()?)
    ...
    Library:GiveSignal(UserInputService.InputChanged:Connect(function(Input: InputObject)
        ...
        if Dragging and IsHoverInput(Input) then
            local CanProceed
            CanProceed, LastResizeInput = ThrottleInput(LastResizeInput)
            if not CanProceed then
                return
            end
...
function Library:AddTooltip(InfoStr: string, DisabledInfoStr: string, HoverInstance: GuiObject)
    ...
    local function DoHover()
        ...
        TooltipLabel.ZIndex = Library.ZIndex.Tooltip
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function InitEvents(Button)
            Button.Base.MouseEnter:Connect(function()
                ...
                Button.Tween = PlayTween(Button.Tween, Button.Base, Library.TweenInfo, {
                    TextTransparency = 0,
                })
            end)
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        function Toggle:Display()
            ...
            Toggle.LabelTween = PlayTween(Toggle.LabelTween, Label, Library.TweenInfo, {
                TextTransparency = Toggle.Value and 0 or 0.4,
            })
            Toggle.ImageTween = PlayTween(Toggle.ImageTween, CheckImage, Library.TweenInfo, {
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        function Toggle:Display()
            ...
            Toggle.BallTween = PlayTween(Toggle.BallTween, Ball, Library.TweenInfo, {
    ...
    function Funcs:AddViewport(Idx, Info)
        ...
        Library:GiveSignal(UserInputService.InputChanged:Connect(function(input)
            ...
            local CanProceed
            CanProceed, LastViewportInput = ThrottleInput(LastViewportInput)
            if not CanProceed then
                return
            end
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:AddTab(...)
        ...
        function Tab:Show()
            ...
            Tab.ShowButtonTween = PlayTween(Tab.ShowButtonTween, TabButton, Library.TweenInfo, {
                BackgroundTransparency = 0,
            })
            Tab.ShowLabelTween = PlayTween(Tab.ShowLabelTween, TabLabel, Library.TweenInfo, {
                TextTransparency = 0,
            })
            if TabIcon then
                Tab.ShowIconTween = PlayTween(Tab.ShowIconTween, TabIcon, Library.TweenInfo, {
                    ImageTransparency = 0,
                })
            end
        ...
        function Tab:Hide()
            Tab.HideButtonTween = PlayTween(Tab.HideButtonTween, TabButton, Library.TweenInfo, {
                BackgroundTransparency = 1,
            })
            Tab.HideLabelTween = PlayTween(Tab.HideLabelTween, TabLabel, Library.TweenInfo, {
                TextTransparency = 0.5,
            })
            if TabIcon then
                Tab.HideIconTween = PlayTween(Tab.HideIconTween, TabIcon, Library.TweenInfo, {
                    ImageTransparency = 0.5,
                })
            end
